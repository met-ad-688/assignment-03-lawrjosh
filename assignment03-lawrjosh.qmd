...
fig = go.Figure(data=[go.Scatter(
    x=grouped_bubble_df["OCCUPATION_NAME"],
    y=grouped_bubble_df["MEDIAN_SALARY"],
    mode='markers',
    marker=dict(
        size=grouped_bubble_df["RECORD_COUNT"],
        color=grouped_bubble_df["MEDIAN_SALARY"], 
        colorscale='Viridis',
        showscale=True,
        sizemode='diameter',
        sizeref=2.*max(grouped_bubble_df["RECORD_COUNT"])/(50.**2),  
        sizemin=4,
        colorbar=dict(title="Median Salary")
    ),
    text=grouped_bubble_df["RECORD_COUNT"],
    textposition="middle center",
    hovertemplate='<b>%{x}</b><br>' +
                  'Median Salary: $%{y:,.0f}<br>' +
                  'Record Count: %{text}<br>' +
                  '<extra></extra>'
)])


fig.update_layout(
    title="Bubble Chart: Occupation vs Median Salary (Bubble Size = Record Count)",
    xaxis_title="Specialized Occupation Name",
